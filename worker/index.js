...
function landletMaxWorldRadius(row) {
  const polygon = JSON.parse(row.polygon_json || '[]');
  if (polygon.length === 0) {
    return Math.hypot(row.center_x_m, row.center_y_m) + Math.sqrt(row.area_m2 / Math.PI);
  }
  return Math.max(...polygon.map((point) => Math.hypot(row.center_x_m + point.x, row.center_y_m + point.y)));
}

function landletMinWorldRadius(row) {
  const polygon = JSON.parse(row.polygon_json || '[]');
  const centerRadius = Math.hypot(row.center_x_m, row.center_y_m);
  if (polygon.length === 0) {
    return Math.max(0, centerRadius - Math.sqrt(row.area_m2 / Math.PI));
  }

  const points = polygon.map((point) => ({ x: row.center_x_m + point.x, y: row.center_y_m + point.y }));
