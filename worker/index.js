...
async function handleWorld(request, db, route) {
  ...
  if (request.method === 'POST' && route.length === 2 && route[1] === 'expand') {
    ...
    const { results } = await db.prepare("SELECT * FROM landlets WHERE status = 'generating'").all();
    ...
    await db.batch([
      ...
      ...enclosed.map((row) => db.prepare(`
        UPDATE landlets
        SET status = 'greenbelt', generated_at = COALESCE(generated_at, strftime('%Y-%m-%dT%H:%M:%fZ', 'now')),
            claimable_at = COALESCE(claimable_at, strftime('%Y-%m-%dT%H:%M:%fZ', 'now')),
            updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
