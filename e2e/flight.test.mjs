...
// right now) spawns already flying, above the world, with no tap at all
// — checked before anything below sets that flag.
const flyingOnFirstEverVisit = await isFlyingClassSet();
const flyBtnActiveOnFirstEverVisit = await isFlyBtnActive();

// The rest of this file is about the manual tap toggle flow, not the
// first-visit spawn above — set the "visited before" flag and reload
...
// Owner (u7rvtz1jn1wchfsz6c1o): a single tap on the bird icon toggles
// flight now — see bindShopFlyToggle in src/main.js (the spacebar
// shortcut just below it still requires a double-press, unaffected here).
...
console.log('flying immediately on a genuinely first-ever visit, no tap (should be true):', flyingOnFirstEverVisit);
