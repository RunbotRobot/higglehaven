...
**Custom avatars as sellable products (N53, #679):** a builder can upload and sell a rigged `.glb` as a normal `avatar`-category catalog product; purchasing one grants the buyer equippable ownership, and Shop mode renders whichever one (if any) they've equipped in place of the default avatar above — see docs/API.md's "Avatar ownership + equip endpoint" and "Rendering the equipped avatar in-world." **Avatar rig/animation clip convention:** the "unified skeleton/animation rig" requirement above is satisfied per-model, not by a single shared skeleton — an uploaded avatar's own `AnimationClip`s named exactly `idle`, `walk`, and/or `fly` are picked up automatically (case-sensitive, matched via `THREE.AnimationClip.findByName`) and crossfaded to match the same grounded-idle/grounded-moving/airborne state the built-in avatar's own procedural pose already follows. None of the three is required: a model missing some or all of them still renders and moves correctly as a rigid whole via root motion (position/rotation on the model's own group), it just doesn't play the missing state's own limb animation — never a hard failure, and never blocking upload or purchase. The upload wizard surfaces which of the three it detected (or that a model has animations under other names) once it has any at all, so a seller doesn't have to equip a model in-world just to discover its clips didn't match.
...
- **Review incentives:** small higgle bonus for genuine, substantive reviews, capped per account/period. **Reviews require a verified purchase (v12 clarification):** standard marketplace practice — only a shopper who actually bought the product can review it.
...
- **Real account login (v13 addition):** email + password, the mainstream-standard baseline (docs/API.md's "Authentication") — the technical account layer this section's age/ID-verification requirements gate. **Age attestation plus the credit-card half of the registration gate is built and enforced (#556):** signup requires attestation, and every builder/seller action requires `trustTier != "none"` — applied to every account, not just new signups (owner decision, Control Room 2026-09-09: "force everyone through the new gates, no grandfathering in"). The government-ID tier (`trustTier: "id_verified"`, via Didit) is #589, not yet built — full social features (voice, messaging, direct communication) still have nothing gating them on that specific requirement until it lands.
...
**Higgles are taxable income at the time earned**, per IRS virtual-currency-as-property framework — regardless of whether spent or converted. A builder earning 10,000 higgles has received $10,000 in ordinary taxable income (same principle as vested-but-unsold employee stock).

**higglehaven's reporting obligations (standard marketplace practice):**
- **W-9 collection timing (resolved recommendation):** requiring a W-9 at signup, before a builder has earned anything, is a real barrier to entry for casual builders who may never approach the $600 reporting threshold — and given higgle-to-cash redemption is itself phased in slowly (§5), there's little practical urgency to front-load this. Recommended approach, matching standard practice on comparable marketplace platforms: **trigger the W-9 requirement once a builder's cumulative trailing earnings reach a threshold comfortably below $600** (e.g., $500), with advance notice, pausing further commission crediting (not confiscating already-earned higgles) until submitted. This clears the compliance requirement before the $600 threshold is ever crossed, without gatekeeping casual/small builders from participating at all.
