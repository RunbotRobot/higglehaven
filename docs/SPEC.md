...
- **Flight:** single tap on the bird icon (mobile/touch UI) / double-press spacebar (desktop keyboard) — owner feedback (2026-09-10): the icon is a deliberately-aimed-at tap target, so it doesn't need the double-press the spacebar shortcut still uses to guard against an accidental stray press. Takeoff ~1s lift + ~1s fade to invisible. Landing: ~2s reverse fade; occupied landing spots offset to nearest open space.
