...
**Earth model (owner decision, #138): a plain sphere, not a real ellipsoid** — the difference is imperceptible at any near-term realistic scale, the same "correct architecture, imperceptible simplification" call already made below for above/below-ground cap asymmetry. **Radius: the IUGG mean Earth radius, 6,371.0 km** (already `DEFAULT_EARTH_RADIUS_M` in `worker/earthCurvature.js`) — the standard citable "Earth's radius" constant, and a better overall real-world fit for a sphere model than either the equatorial or polar radius alone (both of which are biased large/small respectively). See the real-world coordinate origin below for the point this sphere anchors to.
...
- **210-degree rotational offset** applied once during real-world-to-higglehaven coordinate mapping — permanent, zero-cost, thematically resonant detail. **Real-world coordinate origin (owner decision, #138): 2066 McGilvra Blvd. E., Seattle, WA 98112 — 47.638887°N, 122.280433°W.** This is both the point the world's flat (0, 0) origin maps to and the point the 210-degree offset rotates around.
