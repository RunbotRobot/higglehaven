...
delete and all replacement inserts as one batch, so a validation or foreign-key
failure leaves the previous draft intact.

The replacement uses SQLite JSON expansion to insert the validated array in one
statement, keeping the operation to two D1 statements regardless of draft size
(one delete plus one insert) for free-tier efficiency.
...
migrations to isolated test storage, and exercises seed-data reads and starter
landlet claiming. Test storage does not modify the local development D1 state.

## Known gaps / future backend work

- Replace placeholder D1 database IDs in local config when provisioning real
