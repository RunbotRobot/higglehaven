...
**Naming convention:** plain "a" internally in all code/files/DB/APIs (`land`, `landlet`) — the á accent is reserved for user-facing display strings only. The currency term (`higgle`/`higgles`) has no "a" at all, so it's spelled identically everywhere.
